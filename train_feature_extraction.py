...
wght = tf.Variable(tf.truncated_normal(shape, stddev=.01))
bias = tf.Variable(tf.zeros(nb_classes))
logits = tf.matmul(fc7, wght) + bias
...
train_op = opt.minimize(loss_op, var_list=[wght, bias])
